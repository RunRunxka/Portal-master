...
object RemoteCommandHandler {
    ...
    private val needProxyCmd = arrayOf("start", "stop", "set_speed_amp", "set_altitude", "set_speed", "update_location", "set_bearing", "move", "put_config", "start_step_mock", "stop_step_mock", "set_step_count", "set_step_frequency")
    ...
    fun handleInstruction(command: String, rely: Bundle): Boolean {
        ...
        when (commandId) {
            ...
            "start_step_mock" -> {
                FakeLoc.enableMockStep = true
                if (isLoadedLibrary) {
                    val freq = rely.getInt("frequency", FakeLoc.stepFrequency)
                    val baseCount = rely.getLong("base_count", FakeLoc.stepBaseCount)
                    Dobby.setStepMockStatus(true, freq, baseCount)
                }
                return true
            }
            "stop_step_mock" -> {
                FakeLoc.enableMockStep = false
                if (isLoadedLibrary) {
                    Dobby.setStepMockStatus(false, 0, 0)
                }
                return true
            }
            "is_step_mock_start" -> {
                rely.putBoolean("is_step_mock_start", FakeLoc.enableMockStep)
                return true
            }
            "set_step_count" -> {
                val count = rely.getLong("count", FakeLoc.stepBaseCount)
                FakeLoc.stepBaseCount = count
                if (isLoadedLibrary) {
                    Dobby.setStepCount(count)
                }
                return true
            }
            "set_step_frequency" -> {
                val freq = rely.getInt("frequency", FakeLoc.stepFrequency)
                FakeLoc.stepFrequency = freq
                if (isLoadedLibrary) {
                    Dobby.setStepFrequency(freq)
                }
                return true
            }
            "get_location" -> {
            ...
            "put_config" -> {
                ...
                val enableMockStep = rely.getBoolean("enable_mock_step", FakeLoc.enableMockStep)
                val stepFrequency = rely.getInt("step_frequency", FakeLoc.stepFrequency)
                val stepBaseCount = rely.getLong("step_base_count", FakeLoc.stepBaseCount)
                FakeLoc.enableMockStep = enableMockStep
                FakeLoc.stepFrequency = stepFrequency
                FakeLoc.stepBaseCount = stepBaseCount
                if (isLoadedLibrary) {
                    Dobby.setStepMockStatus(enableMockStep, stepFrequency, stepBaseCount)
                }
                return true
            }
            "sync_config" -> {
                ...
                rely.putBoolean("enable_mock_step", FakeLoc.enableMockStep)
                rely.putInt("step_frequency", FakeLoc.stepFrequency)
                rely.putLong("step_base_count", FakeLoc.stepBaseCount)
                return true
            }
            "broadcast_location" -> {
