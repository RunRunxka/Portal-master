...
abstract class BaseDivineService {
    ...
    private fun syncConfig(locationManager: LocationManager, randomKey: String) {
        ...
        if(locationManager.sendExtraCommand("portal", randomKey, rely)) {
            ...
            FakeLoc.enableMockStep = rely.getBoolean("enable_mock_step", FakeLoc.enableMockStep)
            FakeLoc.stepFrequency = rely.getInt("step_frequency", FakeLoc.stepFrequency)
            FakeLoc.stepBaseCount = rely.getLong("step_base_count", FakeLoc.stepBaseCount)
