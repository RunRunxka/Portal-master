...
object MockServiceHelper {
    ...
    fun isStepMockStart(locationManager: LocationManager): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "is_step_mock_start")
        if(locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)) {
            return rely.getBoolean("is_step_mock_start")
        }
        return false
    }

    fun startStepMock(locationManager: LocationManager, frequency: Int, baseCount: Long): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "start_step_mock")
        rely.putInt("frequency", frequency)
        rely.putLong("base_count", baseCount)
        return locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)
    }

    fun stopStepMock(locationManager: LocationManager): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "stop_step_mock")
        return locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)
    }

    fun setStepCount(locationManager: LocationManager, count: Long): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "set_step_count")
        rely.putLong("count", count)
        return locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)
    }

    fun setStepFrequency(locationManager: LocationManager, frequency: Int): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "set_step_frequency")
        rely.putInt("frequency", frequency)
        return locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)
    }
